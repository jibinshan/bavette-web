...
const MenuItemDrawer: FC<MenuItemPopupProps> = ({ children, item }) => {
  ...
  return (
    <Drawer open={open} onOpenChange={setOpen}>
      ...
      <DrawerContent
        className="flex min-h-[520px] w-full flex-col justify-end border-[1px] border-[#212121] bg-[#070707] lg:hidden lg:w-0">
        <div
          style={{
          ...
          className="fixed top-0 w-full h-[230px] rounded-t-xl"
        >

        </div>
        <Button
          className="absolute right-2 top-2 z-50 rounded-full bg-black /50 px-2 py-1"
          variant="ghost"
          onClick={() => setOpen(false)}
        >
          <Icons.close className="h-4 w-4" />
        </Button>
        <div className="flex flex-col h-[600px] px-4 overflow-y-scroll gap-5 z-40">
          <div className="pt-48">
